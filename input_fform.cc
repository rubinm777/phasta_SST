...
// created a k-omega condition in 'Turbulence models' ~line 230
// incorporated a condition to input freestream velocity and characteristic length ~line 437
// modified k-omega condition ~260
// modified turb wall model condition to match SA model (for now) ~ line 750
#include <fstream>
#include <stdio.h>
#include <stdlib.h>
#include <vector>
...
///////////////////////////////////////////////////////////////////////////////
    ...
    if ((string)inp.GetValue("Turbulence Model") == "No-Model" ) {
      ...
      cout << " Turbulence Model: Only Legal Values ( No-Model, LES, RANS-SA, RANS-KE, DES97, DDES, RANS-KOMEGA )";
      cout << endl;
      exit(1);
    }
 ...
 //                                          inp.GetValue("DES Edge Length");
    ...
    if(turbvari.irans == -3) solscalr=solscalr+2;
    if ( solscalr > 4 ) {
...
//      vec.erase(vec.begin(),vec.end());
    ...
    vec = inp.GetValue("Characteristic Veocity for k-w model");
    for(i=0; i< levlset.iLSet +1 ; i++){
      matdat.datmat[i][3][1] = vec[i];
    }
    vec.erase(vec.begin(),vec.end());
   
    vec = inp.GetValue("Characteristic length for k-w model");
    for(i=0; i< levlset.iLSet +1 ; i++){
      matdat.datmat[i][3][2] = vec[i];
    }
    vec.erase(vec.begin(),vec.end());
...
// -----------------------------------------------------------------------------------
    ...
    if( tpturb != 0 ){
      ...
      if (turbvari.irans == -2) turbvar.itwmod = -2;// momdification starts here.
      // ifrule  = inp.GetValue("Velocity Averaging Steps");//not sure if these two lines are necessary
      // turbvar.wtavei =(ifrule >0)? 1.0/ifrule : -1.0/ifrule;
