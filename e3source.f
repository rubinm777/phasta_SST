        !!! k-w model added, but several modifications remain to be introduced.
        !!! k-w model starts around line 596
...
c     note: for iRANS =1, SA model =2, K-epsilon model, =3, K-omega model
c-----------------------------------------------------------------------
        use     turbSA
        use turbKE
        include "common.h"
...
c used locally
        ...
        !MRCHANGE
c    Kay-Epsilon Kay-Omega
        real*8 Ry(npro), Rt(npro), RtP2(npro), f2(npro), f1(npro),
        &       kay(npro), epsilon(npro), omega(npro), fmu(npro), fmui(npro),
...
c.... /start of k-omega model
           ...
           elseif(iRANS.eq.-3) then    ! K-omega
c.... compute the global gradient of u
c
              gradV = zero
              do n = 1, nshl
...
c
        ...
        elseif (iLSet.ne.0) then  !k-w in its current form doesn't
!tolerate multiphase flows. Anything beyond this point is a starting point to begin
!work on SST in multiphase flows (copied from k-epsilon routine). For now, ignore.
           if (isclr.eq.1)  then
...
CAD   follows
       ...
       &                            + gradS(:,3)*gradS(:,3))
           ...
           endif    ! close of scalar 3 of level set
