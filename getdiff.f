c.....modifications start ~line 401 with the introduction of AddEddyViscKW
c.....k-w cases have been added at lines 96 and 161
...
c
      if (iLES .gt. 0 .and. iRANS.eq.0) then   ! simple LES
         ...
         if (iRANS .eq. -1) then ! RANS (Spalart-Allmaras)
            ...
            sigmaInv=1.0        ! use full eddy viscosity for flow equations
            call AddEddyViscKW(yl, dwl, shape, rho, sigmaInv, rmu)
         endif
...
!            sigmaInv=1.0 ! full eddy viscosity for solving kay equation
         ...
         call AddEddyViscKW(yl, dwl, shape, rho, sigmaInv, diffus)
...
c
      ...
      subroutine AddEddyViscKW(yl, dwl, shape, rho, sigmaInv, rmu)
      use turbKW ! access to KE model constants
      include "common.h"
      c     INPUTS
      double precision, intent(in), dimension(npro,nshl,ndof) ::
      &     yl
      ...
      &     rmu
      c     LOCALS
      double precision eviscKE, kay, omega, dw, CmuKE
      double precision omgInv, Rey, Ret, RetInv, tmp1, fmuKE
      integer e,n
      c
      do e = 1, npro
         kay = 0.0
         ...
         eviscKE=rho(e)*ke_C_mu*fmuKE*kay*kay*omgInv
